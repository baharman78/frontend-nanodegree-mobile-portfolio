...
function logAverageFrame(a) {
    ...
    console.log("Average time to generate last 10 frames: " + c / 10 + "ms");
}
...
var adjectives = [ "dark", "color", "whimsical", "shiny", "noise", "apocalyptic", "insulting", "praise", "scientific" ], nouns = [ "animals", "everyday", "fantasy", "gross", "horror", "jewelry", "places", "scifi" ], selectRandomMeat = function() {
    ...
    for (var a = "", b = Math.floor(4 * Math.random()), c = Math.floor(3 * Math.random()), d = Math.floor(2 * Math.random()), e = 0; b > e; e++) a += ingredientItemizer(selectRandomMeat());
    for (var f = 0; c > f; f++) a += ingredientItemizer(selectRandomNonMeat());
    for (var g = 0; d > g; g++) a += ingredientItemizer(selectRandomCheese());
    ...
    b.style.height = "325px", b.id = "pizza" + a, c.classList.add("col-md-6"), d.src = "images/pizza.png", 
    d.classList.add("img-responsive"), c.appendChild(d), b.appendChild(c), e.classList.add("col-md-6"), 
    ...
    function d(a) {
        for (var b = 0; b < document.querySelectorAll(".randomPizzaContainer").length; b++) {
            var d = c(document.querySelectorAll(".randomPizzaContainer")[b], a), e = document.querySelectorAll(".randomPizzaContainer")[b].offsetWidth + d + "px";
            document.querySelectorAll(".randomPizzaContainer")[b].style.width = e;
        }
    }
...
window.addEventListener("scroll", updatePositions), document.addEventListener("DOMContentLoaded", function() {
    for (var a = 8, b = 256, c = 0; 200 > c; c++) {
        var d = document.createElement("img");
        d.className = "mover", d.src = "images/pizza.png", d.style.height = "100px", d.style.width = "73.333px", 
        d.basicLeft = c % a * b, d.style.top = Math.floor(c / a) * b + "px", document.querySelector("#movingPizzas1").appendChild(d);
    }
    updatePositions();
});